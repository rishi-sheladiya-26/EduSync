...
import { getAllNotices } from '../redux/noticeRelated/noticeSlice';
        ...
        const fetchData = async () => {
            try {
                if (currentRole === "Admin") {
                    await dispatch(getAllNotices({ id: currentUser._id, address: "Notice" }));
                } else {
                    await dispatch(getAllNotices({ id: currentUser.school._id, address: "Notice" }));
                }
            } catch (err) {
